...
To make predictions, the app uses the ALBERT (A Lite BERT) model, which can capture the semantic meaning of URLs. The model extracts embeddings, 9,000 of them are then fed into a logistic regression model (`models/base_lr.pkl`). The classification model is then used to classify a given domain name and determine whether it is legitimate or not. 
